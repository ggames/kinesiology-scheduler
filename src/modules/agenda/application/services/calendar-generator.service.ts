...
export class CalendarGeneratorService implements OnModuleInit {
  ...
  async ensureSeedTemplatesAndClinics(): Promise<{ clinic: Clinic; templatesCreated: number }> {
    ...
    if (weeklyScheduleCount === 0) {
      this.logger.log('Seeding default weekly schedules (Mon-Thu 08:00-20:00, Fri 08:00-18:00) for clinic...');
      ...
      for (let dayOfWeek = 1; dayOfWeek <= 5; dayOfWeek++) {
        weeklySchedules.push(
          this.weeklyScheduleRepo.create({
            ...
            dayOfWeek,
            startTime: '08:00:00',
            endTime: dayOfWeek === 5 ? '18:00:00' : '20:00:00',
    ...
    for (const prof of professionals) {
      ...
      if (existingTemplates === 0) {
        this.logger.log(`Seeding default schedule template for professional ${prof.id}...`);
    ...
    if (holidayCount === 0) {
      this.logger.log('Seeding default holidays and asuetos...');
      const sampleHolidays = [
        { date: '2026-09-25', type: HolidayType.TOTAL, description: 'Asueto Administrativo de la Clínica', clinic },
        { date: '2026-10-12', type: HolidayType.TOTAL, description: 'Día del Respeto a la Diversidad Cultural (Feriado Nacional)', clinic },
        { date: '2026-11-20', type: HolidayType.TOTAL, description: 'Día de la Soberanía Nacional (Feriado Nacional)', clinic },
        { date: '2026-12-08', type: HolidayType.TOTAL, description: 'Día de la Inmaculada Concepción (Feriado Nacional)', clinic },
        { date: '2026-12-25', type: HolidayType.TOTAL, description: 'Navidad (Feriado Nacional)', clinic },
      ];
      await this.holidayRepo.save(sampleHolidays as any);
    }
